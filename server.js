...
let serviceAccount = require("./ecomm-62f20-firebase-adminsdk-cpgah-dc646bcaf0.json");
...
const bucketName = "ecom-electronics-beni";
...
app.post('/add-product', (req, res) => {
    let {name, shortDes, des, images, colors, actualPrice, discount, sellPrice, stock, tags, tac, email, draft, id} = req.body;
    ...
    if(!draft){
        ...
        } else if(!colors.length){ // color array
            return res.json({'alert': 'select at least one size'});
        } else if(!actualPrice.length || !discount.length || !sellPrice.length){
            return res.json({'alert': 'you must add prices'});
        } else if(stock < 10){
            return res.json({'alert': 'you should have at least 10 items in stock'});
        } else if(!tags.length){
...
app.post('/order', (req, res) => {
    ...
    const mailOption = {
        from: 'sender email',
        to: email,
        subject: 'Electronics : Order Placed',
        html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
